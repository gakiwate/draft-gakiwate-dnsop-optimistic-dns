...
DNS lookups introduce user-visible latency, particularly when cached
records have expired and must be refreshed from the network.  This
document describes Optimistic DNS, a client-side stub resolver mechanism
that immediately returns expired cached DNS records to applications while
simultaneously refreshing them with a network query.  The application
receives a usable answer in microseconds rather than milliseconds, and
receives an updated answer shortly thereafter if the data has changed.

Optimistic DNS is complementary to RFC 8767, which addresses serving
stale data at recursive resolvers.  This document focuses exclusively on
client-side stub resolver behavior, including explicit signaling between
the stub resolver and applications about answer freshness.
...
application that asks for www.example.com gets an instant answer.  At time
T=61, the record has expired.  The very next lookup must go to the network.
From the user's perspective, the transition from "instant" to "slow" is a
cliff edge:

| Time (seconds) | Response Latency        |
|-----------------|-------------------------|
| T=0 (fresh)     | ~0 ms (cache hit)       |
| T=30 (fresh)    | ~0 ms (cache hit)       |
| T=59 (fresh)    | ~0 ms (cache hit)       |
| T=61 (expired)  | 150 ms (network query)  |
| T=62 (fresh)    | ~0 ms (cache hit again) |

The record was valid for 60 seconds, then invalid for the fraction of a
second it took to refresh it, then valid again.  Yet that fraction of a
second is the one the user noticed.

This document describes Optimistic DNS, a stub resolver mechanism that
addresses this problem.  When the stub resolver has expired cached records
that match a query, it returns those expired records to the application
immediately -- within microseconds -- while simultaneously issuing a fresh
network query in the background.  The application receives two sets of
results: first the expired (but likely still correct) records, then the
authoritative fresh records.

The application is explicitly told which answers are expired and which are
fresh, allowing it to make intelligent decisions about how to use each.  An
application connecting to a well-known server may immediately use the
expired address and establish a connection, gambling that the address has
not changed.  An application performing a security-sensitive lookup may
choose to wait for the fresh answer.
...
Optimistic DNS is complementary to the mechanism described in
{{!RFC8767}}, which allows recursive resolvers to serve stale data during
upstream failures.  The two mechanisms address different parts of the
...
: A cached DNS record whose TTL has reached zero.  Under conventional DNS
  ...
  when answering queries.  Optimistic DNS gives expired records a second
  life.
...
: An expired cached record that is returned to an application in response
  ...
  data is still current.  Optimistic answers are explicitly flagged as
  expired so the application can distinguish them from fresh answers.
...
Mortal Record
: A cached DNS record in its normal lifecycle state.  When its TTL
  expires, a mortal record is eligible for purging from the cache.  This
...
: An immortal record whose TTL has expired.  Ghost records linger in the
  cache for an extended period (the ghost retention period) and can serve
  as optimistic answers to future queries.  After the ghost retention
  period elapses, the ghost record is finally purged.

Ghost Retention Period
: The maximum time a ghost record is retained in the cache after its TTL
  has expired.  This document recommends a default of one week.
...
TTL Stretching is a simple modification to the stub resolver behavior.
In TTL stretching, the stub resolver unilaterally extends the effective
...
: The application cannot distinguish a fresh record from a stale one.
  This makes it impossible for the application to make risk-appropriate
  decisions.  A web browser rendering a non-critical image might be happy
  to use a stretched record without question, while a security-sensitive
  authentication flow might prefer to wait for a confirmed-fresh answer.
  TTL stretching forces both applications into the same behavior.

No opt-in or opt-out
: All applications receive stretched TTLs whether they want them or not.
  An application that specifically needs authoritative-fresh data (for
  example, to verify that a DNS-based access control record has been
  updated) has no way to bypass the stretched cache.

Bounded effectiveness
: TTL stretching is practical only for brief periods of staleness.  A
  stretch window of a few minutes handles the common case of a record
  expiring between successive page loads.  But it does not help after
  longer gaps -- for example, when a laptop wakes from an overnight
  sleep and all cached records expired hours ago.  Extending the stretch
  window to cover these cases would require serving records that are
  hours or days stale, significantly increasing the risk of returning
  incorrect data with no way for the application to detect it.

Optimistic DNS, which we discuss next, generalizes TTL stretching by making
staleness explicit.  The application receives expired records just as quickly,
but with a clear signal that the data may be stale.  This transparency allows
Optimistic DNS to extend the mechanism to much longer time horizons (via
immortalization and ghost records) while keeping the application informed and in
control.
...
1. It immediately returns the expired cached records to the application,
   flagging each one as an expired answer.
...
the application through the normal callback mechanism.

The application therefore receives two waves of results:

First wave (immediate)
: Expired cached records, explicitly marked as expired.  These arrive
  within microseconds of the query being issued.

Second wave (after network round trip)
: Fresh records from the network.  These arrive after the normal DNS
  resolution delay.

The following diagram shows both waves for a query where www.example.com
...
| T+5us  | Callback: 93.184.216.34        | Expired, FromCache       |
| T+5us  | Signal: no more expired records|                          |
| T+120ms| Callback: 93.184.216.34        | (fresh, from network)    |

In this case the expired answer and the fresh answer contain the same
data.  The application that began connecting at T+5us saved 120
...
| T+5us  | Callback: 93.184.216.34        | Expired, FromCache       |
| T+5us  | Signal: no more expired records|                          |
| T+120ms| Callback: 198.51.100.42        | (fresh, from network)    |
...
The key insight is that Optimistic DNS never makes things worse for the
application.  In the common case where the data has not changed, it makes
things dramatically faster.  In the uncommon case where the data has
changed, it costs nothing beyond a failed connection attempt that overlaps
with the network query the resolver would have performed anyway.
...
matter between the application and the stub resolver -- it is not carried
on the wire.

The stub resolver API provides a flag (called AllowExpiredAnswers in this
document) that the application includes with its query.  Setting this flag
indicates that the application is prepared to handle expired answers and
will treat them appropriately.

Upon receiving a query with AllowExpiredAnswers set, the stub resolver sets
its internal expired record policy for that query to "UseCached".  This
policy has two effects:

1. Expired positive cache records matching the query will be returned to
   the application, flagged as expired.

2. All records that answer this query -- whether cached or received from
   the network -- will be immortalized (see {{record-immortalization}}).

If the global Optimistic DNS feature is disabled in the stub resolver's
configuration, the AllowExpiredAnswers flag is ignored and the query
proceeds conventionally.

## Cache Lookup with Expired Records

When the stub resolver processes a query with the UseCached policy, it
iterates through its cache looking for records that match the query's name,
type, and class.  For each matching record, it determines whether the
record has expired by comparing the time elapsed since the record was
...
Each expired record that is returned to the application is accompanied by
the ExpiredAnswer flag in the callback, allowing the application to
distinguish it from a fresh cached answer.  The AnsweredFromCache flag is
also set, since the record did come from the cache (albeit an expired
portion of it).

## Parallel Network Query

Regardless of whether expired records were found in the cache, the stub
resolver issues a standard DNS query on the network.  This query proceeds
through the normal resolution path: contacting configured recursive
resolvers, following CNAME chains, appending search domains if applicable,
and so on.

Fresh answers from the network are delivered to the application through the
normal callback mechanism, without the ExpiredAnswer flag.  The application
can use these fresh answers to confirm or replace any expired answers it
received earlier.

The parallel network query is not optional.  Even if expired records were
returned from the cache, the network query MUST still be issued.  The
expired records are a convenience for the application, not a substitute for
proper DNS resolution.

## Record Immortalization {#record-immortalization}

A critical component of Optimistic DNS is the record lifecycle management
that ensures expired records remain available for future optimistic
queries.  Without this mechanism, expired records would be purged from the
cache according to normal TTL rules, and Optimistic DNS would have nothing
to offer.

Optimistic DNS extends the standard record lifecycle with two additional
states beyond the normal "mortal" state:

Mortal (default)
: A record in the normal cache lifecycle.  When its TTL expires, it is
  eligible for immediate removal from the cache.  All newly cached records
  begin in this state.

Immortal
: A record that has been marked to survive past its TTL expiry.  A mortal
  record is promoted to immortal when it answers a query that has the
  Optimistic DNS policy active (either UseCached or Immortalize).  The
  promotion occurs at the time the record is delivered to the application,
  whether the record is expired or fresh.

  The following conditions must all be true for a record to be
  immortalized:

  - The query's expired record policy is UseCached or Immortalize.
  - The record is a positive record (not a negative cache entry).
  - The record is not DNSSEC-validated (see {{dnssec}}).
  - The record is being added (not removed) from the answer set.

Ghost
: When an immortal record's TTL expires, instead of being purged, it
  transitions to the ghost state.  A ghost record is an expired immortal
  record that lingers in the cache, available to serve future optimistic
  queries.

  Ghost records are retained for a bounded period called the ghost
  retention period.  This document recommends a default ghost retention
  period of one week.  After the ghost retention period elapses, the ghost
  record is finally purged from the cache like any other expired record.
...
The immortalization mechanism ensures that once a record has been used to
answer an optimistic query, it remains available in the cache for a
substantial period.  This creates a virtuous cycle: the more frequently a
name is queried with Optimistic DNS, the more likely it is that the cache
will contain a ghost record for it the next time an optimistic query
arrives after the TTL has expired.

## Policy Transition

After the stub resolver has delivered all available expired records from
the cache and there are no more expired records to return, it performs two
actions:

1. It signals the application that no more expired records will be
   provided.  This signal is delivered through the query's event handler
   mechanism.

2. It downgrades the query's expired record policy from UseCached to
   Immortalize.

The Immortalize policy preserves the immortalization behavior (fresh
answers that arrive from the network will still be immortalized) but
disables the use of expired records.  This ensures that later processing
stages (such as CNAME following) do not inadvertently serve expired records
that were not part of the initial cache lookup.

The three expired record policies form a strict ordering:

DoNotUse
: The default for queries without AllowExpiredAnswers.  Expired records are
  ignored.  Records are not immortalized.

UseCached
: The initial policy for optimistic queries.  Expired records are returned.
  Records are immortalized.

Immortalize
: The downgraded policy after expired records have been delivered.  Expired
  records are no longer returned, but records are still immortalized.

Policy transitions are one-directional: UseCached transitions to
Immortalize, and Immortalize remains at Immortalize.  A query never
transitions back to a more permissive policy.
...
1. The expired CNAME is returned to the application, flagged as expired,
   so the application has the information.

2. When the "no more expired records" signal is generated, the resolver
   detects that an expired CNAME was encountered.

3. The resolver rewinds the query back to the original QNAME
   (www.example.com in the example above).

4. The query is restarted with the Immortalize policy instead of
   UseCached, meaning no more expired records will be returned but fresh
   answers will still be immortalized.

5. The restarted query proceeds through normal DNS resolution, following
   whatever CNAME chain the fresh network response provides.
...
When search domains are in use (the resolver is appending search domains
to a partially-qualified name), the restart preserves the current search
domain.  For example, if the resolver was trying
www.example.com.corp.example.net (the original query www with search domain
corp.example.net) when the expired CNAME was encountered, the restart
continues with that same search domain rather than advancing to the next
one in the list.

# Signaling

Optimistic DNS uses local signaling between the application and the stub
resolver.  No new DNS wire-protocol elements are defined.

## Query Flags

AllowExpiredAnswers
: Set by the application when issuing a query to indicate that the
  application is willing to receive expired cached records.  This flag is
  purely local; it is not transmitted on the wire.

  When this flag is set, and the stub resolver's global Optimistic DNS
  feature is enabled, the query's expired record policy is set to
  UseCached.

  When this flag is not set, the query behaves conventionally: only
  unexpired cached records and fresh network records are returned.

## Response Flags

ExpiredAnswer
: Set by the stub resolver on individual answer callbacks to indicate that
  the record being returned is an expired cached record.  The application
  SHOULD treat this answer as potentially stale and be prepared for the
  data to be incorrect.

  This flag is set when the record returned is a ghost record (an immortal
  record that has expired) or when an expired CNAME was encountered during
  resolution.

AnsweredFromCache
: Set by the stub resolver to indicate that the record was served from the
  local cache rather than from a network response.  This flag is set
  regardless of whether the record is expired, allowing the application to
  distinguish between cached answers (immediate) and network answers
  (delayed).

NoMoreExpiredRecords (event)
: A signal delivered through the query's event mechanism to indicate that
  the stub resolver has finished delivering all available expired cached
  records.  After this signal:

  - No more expired records will be returned for this query.
  - The query's expired record policy is downgraded to Immortalize.
  - If an expired CNAME was encountered, the query is restarted at the
    original QNAME (see {{cname-handling}}).
...
When the stub resolver is processing a query that requests DNSSEC
validation, records MUST NOT be immortalized, and expired records MUST NOT
be returned, regardless of the AllowExpiredAnswers flag.

Similarly, records that were originally received with DNSSEC validation
MUST NOT be immortalized even when answering a non-DNSSEC query, because
doing so could cause them to be served as optimistic answers to a future
DNSSEC-requesting query after their signatures have expired.
...
The transport used to obtain a record has no bearing on whether that record
can be immortalized or served as an expired answer.  The cache does not
distinguish between records obtained via different transports.
...
3. The "no more expired records" signal fires, and the resolver detects the
   expired CNAME.
4. The resolver restarts at mail.corp.example.com (not mail.example.com)
   with the Immortalize policy.
5. The fresh network query resolves mail.corp.example.com normally.

## Relationship to RFC 8767

{{!RFC8767}} ("Serving Stale Data to Improve DNS Resiliency") describes a
...
A key difference is in the signaling model.  RFC 8767 serves stale data
transparently -- the stub resolver cannot distinguish a stale answer from a
fresh one based on the DNS wire protocol.  In this regard, RFC 8767 is
more similar to TTL stretching ({{ttl-stretching}}) than to Optimistic
DNS: both RFC 8767 and TTL stretching serve stale data without informing
the application.  Optimistic DNS, by contrast, explicitly signals
staleness to the application through local API flags, empowering the
application to make informed decisions.

# Security Considerations

Optimistic DNS introduces a tradeoff between latency and freshness.
Applications that use expired answers must be prepared for those answers to
be incorrect.
...
: If an attacker successfully poisons a cache entry, the immortalization
  mechanism could extend the lifetime of the poisoned record beyond its
  original TTL.  However, the ghost retention period provides a bounded
  window of exposure.  The record will eventually be purged regardless of
  its mortality state.  Additionally, the parallel network query will
  obtain and deliver the correct answer, giving the application an
  opportunity to detect the discrepancy.

DNSSEC Exclusion
: As described in {{dnssec}}, Optimistic DNS excludes DNSSEC-validated
  records from immortalization and expired serving.  This prevents the
  serving of records whose cryptographic signatures have expired,
  maintaining DNSSEC's security guarantees.

Ghost Retention Period
: The ghost retention period (recommended default: one week) bounds the
...
Negative Record Exclusion
: Negative cache entries (representing NXDOMAIN or NODATA responses) are
  excluded from optimistic serving.  This prevents stale negative answers
  from suppressing the discovery of newly-created names.

Applications using Optimistic DNS SHOULD employ transport-layer security
(TLS, QUIC) when connecting to addresses obtained from expired
answers, to detect cases where the address has been reassigned.
