...
While connecting, the networking library code MUST continue to pay attention
...
yet Recursive B happily sent it cached data that was no longer correct.

If the data was already considered “expired” at time T=80, it might be
...
1. If the record is expired, and a background query is not already in
   progress for this record name, then a background query is initiated,
   as for any other expired record.
...
changes may mean that the CNAME chain for a given name changes over time.

# Interaction with Other DNS Features

## Uncacheable Records

{{Section 3.2.1 of !RFC1035}} specifies that DNS records
with a TTL of zero "can only be used for the transaction in progress,
and should not be cached." This document does not change that guidance;
...
Applications using Optimistic DNS SHOULD employ application-layer
...
Networking always entails a degree of uncertainty
...
: The normal cache state for a record retrieved to
  ...
  of normal operation,
  though because of Zeno’s paradox ({{zeno}})
