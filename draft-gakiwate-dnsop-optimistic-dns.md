...
: A DNS resolution model where the application initiates a query and
  ...
  The mental model of asynchronous DNS resolution is that
  it is a performance optimization over rapid polling.
  ...
  Most consecutive calls to getaddrinfo() would return the same information.
  ...
  The application is notified via an asynchronous notification
  only when the set of answers changes.
